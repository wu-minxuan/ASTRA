...
Phase 1 初始 P1-T07 可以先不实现 WebKnowledgeProvider，但 P1-T07 证据补全、P1-T08 模型粗排、P1-T09 模型精排和 P1-T10 报告生成的设计应为该 provider 预留扩展点。

若 Phase 1 后续实现最小网页证据能力，应优先采用 mock/search fixture 测试，不让默认自动化测试依赖真实搜索引擎、实时网页或不稳定网络。

P1-O05 已实现最小 `WebKnowledgeProvider` 合同和 AKShare-backed 实现。该实现不是通用搜索引擎抓取，也不让模型自行联网；它只把 AKShare 可追踪入口中的新闻、公告和研报记录转换为结构化 evidence：

- `stock_news_em`：个股新闻，映射为 `source_type=news`。
- `stock_individual_notice_report`：个股公告，映射为 `source_type=public_disclosure`。
- `stock_research_report_em`：个股研报，映射为 `source_type=research_report`。

P1-O05 的默认单元测试使用 fake/mock WebKnowledgeProvider，不访问真实网络。真实 AKShare WebKnowledge 验证通过 `make test-live-akshare` 单独运行；任一来源失败时应记录 warning，并保留其他成功来源生成的 evidence。
